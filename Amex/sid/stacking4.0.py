...
for m,stkr in enumerate(stacker):
	...
	for j,(train,test) in enumerate(skf):
		print "Stacker FOLD: ",j+1
		stkr.fit(X_training[train],y_training[train])

		X_blended_training[test[:, None],columns] = stkr.predict_proba(X_training[test])
		X_blended_submission_stkr = X_blended_submission_stkr + stkr.predict_proba(X_submission)

		X_blended_submission_stkr = X_blended_submission_stkr/len(skf)
	X_blended_submission[:,columns] = X_blended_submission_stkr

print X_blended_training.shape	
print X_blended_submission.shape
print 'Saving X_blended_training,X_blended_submission'
np.save('X_blended_training_stack.npy',X_blended_training)
np.save('X_blended_submission_stack.npy',X_blended_submission)
print 'Data saved as binary format'
print 'Blending/Stacking Done'
print "Cross Validating"
...
# final_stacker=LinearSVC(C=0.1, penalty="l1", dual=False)
...
accuracy = final_stacker.score(X_training,y_training)
pred = final_stacker.predict(X_training)
last = X_training[:,0]
print last.shape
print "Cross validation accuracy", np.mean(pred == y_training)
...
if accuracy > 0.85:
	...
	for i in range(len(data)):
		...
		final.append(row)
	with open('Dunjen_master_IITKharagpur_stacked4.0.csv','wb') as f:
		a = csv.writer(f) 
		a.writerows(final)
	f.close()
